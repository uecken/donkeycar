#!/usr/bin/env python3
"""
AIデバッグ機能付き manage.py
=============================
作成: robotcar-engineer + ml-engineer
目的: AI出力値（pilot/angle, pilot/throttle）をリアルタイムでターミナルに表示
...
変更点:
    - DebugPilotクラスを追加: AI出力値をリアルタイム表示
    - AI_DEBUG設定をサポート（myconfig.pyで有効/無効切替）
"""
...
# ==============================================================================
# DebugPilot: AI出力値をリアルタイムでターミナルに表示するパーツ
# ==============================================================================
class DebugPilot:
    """
    AIの出力値（angle, throttle）をリアルタイムでターミナルに表示するパーツ。

    表示内容:
    - モード（user/local_angle/local）
    - pilot/angle と pilot/throttle
    - PWM変換値（HBX 2192設定）
    - 動作解釈（前進/後退/停止、左旋回/直進/右旋回）
    """

    def __init__(self, cfg=None, show_pwm=True, throttle_mult=1.0):
        """
        Args:
            cfg: 設定オブジェクト（オプション）
            show_pwm: PWM値を表示するかどうか
            throttle_mult: スロットル乗数（AI_THROTTLE_MULT）
        """
        self.show_pwm = show_pwm
        self.throttle_mult = throttle_mult
        self.frame_count = 0
        self.last_print_time = 0

        # HBX 2192 PWM設定（myconfig.pyから取得する場合はcfgを使用）
        if cfg:
            self.THROTTLE_FORWARD = getattr(cfg, 'THROTTLE_FORWARD_PWM', 330)
            self.THROTTLE_STOPPED = getattr(cfg, 'THROTTLE_STOPPED_PWM', 307)
            self.THROTTLE_REVERSE = getattr(cfg, 'THROTTLE_REVERSE_PWM', 281)
            self.STEERING_LEFT = getattr(cfg, 'STEERING_LEFT_PWM', 280)
            self.STEERING_RIGHT = getattr(cfg, 'STEERING_RIGHT_PWM', 330)
        else:
            # デフォルト値（HBX 2192）
            self.THROTTLE_FORWARD = 330
            self.THROTTLE_STOPPED = 307
            self.THROTTLE_REVERSE = 281
            self.STEERING_LEFT = 280
            self.STEERING_RIGHT = 330

        self.STEERING_CENTER = (self.STEERING_LEFT + self.STEERING_RIGHT) // 2

    def run(self, mode, pilot_angle, pilot_throttle, user_angle, user_throttle):
        """
        AI出力値をターミナルに表示する。

        Args:
            mode: 現在のモード（'user', 'local_angle', 'local'）
            pilot_angle: AIが出力したステアリング角度 (-1.0 ~ +1.0)
            pilot_throttle: AIが出力したスロットル値 (-1.0 ~ +1.0)
            user_angle: ユーザー入力のステアリング角度
            user_throttle: ユーザー入力のスロットル値

        Returns:
            pilot_angle, pilot_throttle をそのまま返す（パススルー）
        """
        self.frame_count += 1

        # 自動運転モードの場合のみ詳細表示
        if mode in ['local', 'local_angle']:
            # 実際に使用される値を計算
            if mode == 'local':
                actual_angle = pilot_angle if pilot_angle else 0.0
                actual_throttle = (pilot_throttle * self.throttle_mult) if pilot_throttle else 0.0
            else:  # local_angle
                actual_angle = pilot_angle if pilot_angle else 0.0
                actual_throttle = user_throttle if user_throttle else 0.0

            # PWM変換
            if actual_throttle >= 0:
                throttle_pwm = int(actual_throttle * (self.THROTTLE_FORWARD - self.THROTTLE_STOPPED) + self.THROTTLE_STOPPED)
            else:
                throttle_pwm = int((actual_throttle + 1) * (self.THROTTLE_STOPPED - self.THROTTLE_REVERSE) + self.THROTTLE_REVERSE)

            if actual_angle >= 0:
                steering_pwm = int(actual_angle * (self.STEERING_RIGHT - self.STEERING_CENTER) + self.STEERING_CENTER)
            else:
                steering_pwm = int((actual_angle + 1) * (self.STEERING_CENTER - self.STEERING_LEFT) + self.STEERING_LEFT)
            ...
            if actual_angle < -0.3:
                steer_dir = "LEFT"
            elif actual_angle > 0.3:
                steer_dir = "RIGHT"
            else:
                steer_dir = "STRAIGHT"

            if actual_throttle > 0.1:
                throttle_dir = "FWD"
            elif actual_throttle < -0.1:
                throttle_dir = "REV"
            else:
                throttle_dir = "STOP"

            # モード表示
            mode_str = "FULL_AUTO" if mode == 'local' else "AUTO_STEER"

            # 出力フォーマット
            if self.show_pwm:
                output = (
                    f"\r[{mode_str}] "
                    f"angle:{pilot_angle:+.3f}({steering_pwm}) "
                    f"throttle:{pilot_throttle:+.3f}*{self.throttle_mult:.1f}={actual_throttle:+.3f}({throttle_pwm}) "
                    f"[{steer_dir}/{throttle_dir}] "
                    f"frame:{self.frame_count}"
                )
            else:
                output = (
                    f"\r[{mode_str}] "
                    f"angle:{pilot_angle:+.4f} "
                    f"throttle:{pilot_throttle:+.4f} (actual:{actual_throttle:+.4f}) "
                    f"[{steer_dir}/{throttle_dir}]"
                )
            ...
            # userモードでは簡易表示（数フレームに1回）
            if self.frame_count % 20 == 0:
                print(f"\r[USER] angle:{user_angle:+.3f} throttle:{user_throttle:+.3f} frame:{self.frame_count}", end='', flush=True)

        return pilot_angle, pilot_throttle


# ==============================================================================
# 以下は complete.py からの標準クラス
# ==============================================================================
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    DebugPilot added for AI output monitoring.
    """
    ...
    if model_path:
        ...
        V.add(FileWatcher(model_path), outputs=['modelfile/dirty'], run_condition="run_pilot")
        V.add(DelayedTrigger(100), inputs=['modelfile/dirty'], outputs=['modelfile/reload'], run_condition="run_pilot")
        V.add(TriggeredCallback(model_path, model_reload_cb), inputs=["modelfile/reload"], run_condition="run_pilot")
    ...
    # ==========================================================================
    # DebugPilot追加: AI出力値をリアルタイム表示
    # ==========================================================================
    ai_debug_enabled = getattr(cfg, 'AI_DEBUG', True)  # デフォルトで有効
    if ai_debug_enabled:
        print("\n" + "=" * 60)
        print("AI DEBUG MODE ENABLED")
        print("=" * 60)
        print("AI output values will be displayed in real-time.")
        print("To disable: add 'AI_DEBUG = False' to myconfig.py")
        print("=" * 60 + "\n")

        debug_pilot = DebugPilot(
            cfg=cfg,
            show_pwm=True,
            throttle_mult=getattr(cfg, 'AI_THROTTLE_MULT', 1.0)
        )
        V.add(debug_pilot,
...
def add_imu(V, cfg):
    ...
    if cfg.HAVE_IMU:
        ...
        imu = IMU(sensor=cfg.IMU_SENSOR, addr=cfg.IMU_ADDRESS, dlp_setting=cfg.IMU_DLP_CONFIG)
