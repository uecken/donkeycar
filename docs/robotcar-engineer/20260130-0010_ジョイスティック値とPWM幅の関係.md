...
## 7. HBX 2192 PWM設定値の検証

### 7.1 現在の設定値（myconfig.py）

```python
"THROTTLE_FORWARD_PWM": 330,            # 1370us ← コメント誤り！
"THROTTLE_STOPPED_PWM": 307,            # 1500us ← 正しい
"THROTTLE_REVERSE_PWM": 281,            # 1610us ← コメント誤り！
```
...
| 設定項目 | PWM値 | 実際のパルス幅 | コメント | 判定 |
|---------|-------|---------------|---------|------|
| `THROTTLE_FORWARD_PWM` | 330 | **1611μs** | 1370us | **❌ 誤り** |
| `THROTTLE_STOPPED_PWM` | 307 | **1499μs** | 1500us | ✅ 正しい |
| `THROTTLE_REVERSE_PWM` | 281 | **1372μs** | 1610us | **❌ 誤り** |

**計算**:
```
PWM 330: 330 / 4096 × 20000 = 1611μs（前進最大）
PWM 307: 307 / 4096 × 20000 = 1499μs（停止）
PWM 281: 281 / 4096 × 20000 = 1372μs（後退最大）
```

### 7.3 コメントが入れ替わっている

| 設定項目 | PWM値 | 正しいパルス幅 | 意味 |
|---------|-------|---------------|------|
| `THROTTLE_FORWARD_PWM` | 330 | **1611μs** | 前進（パルス幅が長い = 速度大） |
| `THROTTLE_STOPPED_PWM` | 307 | **1499μs** | 停止（ニュートラル ~1500μs） |
| `THROTTLE_REVERSE_PWM` | 281 | **1372μs** | 後退（パルス幅が短い） |

**結論**: PWM値自体は正しいが、**コメントのμs値が入れ替わっている**。
...
**評価**: ESCの動作範囲が狭い（1372〜1611μs）が、標準的な方向性（長いパルス = 前進）は正しい。

### 7.5 修正されたコメント

```python
# 正しいコメント
"THROTTLE_FORWARD_PWM": 330,            # 1611μs（前進最大）
"THROTTLE_STOPPED_PWM": 307,            # 1499μs（停止）
"THROTTLE_REVERSE_PWM": 281,            # 1372μs（後退最大）
```

### 7.6 PWM値と実効速度の関係

`JOYSTICK_MAX_THROTTLE` が速度制限として機能する際の影響:

| `JOYSTICK_MAX_THROTTLE` | throttle最大値 | 前進時PWM | 前進時パルス幅 | 実効速度 |
|-------------------------|---------------|-----------|---------------|---------|
| 1.0 | ±1.0 | 330 | 1611μs | 100% |
| 0.8 | ±0.8 | 325 | 1587μs | 80% |
| 0.5 | ±0.5 | 318 | 1553μs | 50% |
| 0.3 | ±0.3 | 314 | 1533μs | 30% |

**計算例（JOYSTICK_MAX_THROTTLE = 0.5）**:
```
throttle = 0.5
PWM = map_range(0.5, 0, 1, 307, 330)
    = 0.5 × (330 - 307) + 307
    = 0.5 × 23 + 307
    = 318.5 → 318
パルス幅 = 318 / 4096 × 20000 = 1553μs
```
...
def calculate_throttle_pwm(throttle, zero_pulse, max_pulse, min_pulse):
    """throttle値からPWM値を計算（PWMThrottleと同じロジック）"""
    if throttle > 0:
        return int(throttle * (max_pulse - zero_pulse) + zero_pulse)
    else:
        return int((throttle + 1) * (zero_pulse - min_pulse) + min_pulse)
...
print("=== HBX 2192 PWM設定検証 ===")
print(f"THROTTLE_FORWARD_PWM = {FORWARD} → {pwm_to_us(FORWARD):.0f}μs")
print(f"THROTTLE_STOPPED_PWM = {STOPPED} → {pwm_to_us(STOPPED):.0f}μs")
print(f"THROTTLE_REVERSE_PWM = {REVERSE} → {pwm_to_us(REVERSE):.0f}μs")

print("\n=== JOYSTICK_MAX_THROTTLE による制限 ===")
for max_throttle in [1.0, 0.8, 0.5, 0.3]:
    pwm = calculate_throttle_pwm(max_throttle, STOPPED, FORWARD, REVERSE)
    print(f"MAX_THROTTLE={max_throttle}: PWM={pwm}, パルス幅={pwm_to_us(pwm):.0f}μs")
```

**出力例**:
```
=== HBX 2192 PWM設定検証 ===
THROTTLE_FORWARD_PWM = 330 → 1611μs
THROTTLE_STOPPED_PWM = 307 → 1499μs
THROTTLE_REVERSE_PWM = 281 → 1372μs

=== JOYSTICK_MAX_THROTTLE による制限 ===
MAX_THROTTLE=1.0: PWM=330, パルス幅=1611μs
MAX_THROTTLE=0.8: PWM=325, パルス幅=1587μs
MAX_THROTTLE=0.5: PWM=318, パルス幅=1553μs
MAX_THROTTLE=0.3: PWM=314, パルス幅=1533μs
```
...
| 項目 | 状態 |
|------|------|
| PWM値（330/307/281） | ✅ **正しい** |
| コメントのμs値 | ❌ **入れ替わっている** |
| ESC動作範囲 | ✅ 標準的な方向性（長いパルス = 前進） |
| `JOYSTICK_MAX_THROTTLE` との連携 | ✅ 正常に速度制限として機能 |

**推奨アクション**: myconfig.py のコメントを修正する（PWM値自体は変更不要）
...
| 2026-01-30 00:20 | HBX 2192 PWM設定値の検証セクション追加（コメントのμs値入れ替わり問題を特定） |
