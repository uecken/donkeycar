...
HBX 2192付属の赤基板ESCをそのまま利用し、PCA9685経由でPWM制御。
...
Raspberry Pi 4
    ...
    └── I2C (GPIO 2,3)
            │
            └── PCA9685 (0x40)
                    ├── Ch0 ──→ ステアリングサーボ（5線式、PWM対応確認済み）
                    └── Ch1 ──→ 赤基板ESC ──→ 370モーター
```

- I2Cアドレス: 0x40（PCA9685デフォルト）
- PWM周波数: 50Hz（サーボ/ESC標準）
- ESC動作範囲: 1370〜1610μs（PWM値 281〜330）
...
### 2.4 ESC起動時の突入電流問題（実証済み）

**問題**: モーターと接続した状態でESCに電源を入れると、モーターの突入電流（インラッシュ電流）によりESCが正常起動しない。
...
|------|----------|
| **正常起動** | 「ピッピピピ」と確認音が鳴る |
| **起動失敗** | モーターが「カカカカ...」と小刻みに動き、その後停止 |

#### 原因

モーターは起動時に定常電流の数倍〜10倍の突入電流が流れる。この突入電流により：
1. 電源電圧が瞬間的に低下
2. ESCの内部回路がリセットまたは誤動作
3. 正常な初期化シーケンスが完了しない

#### 解決策（実証済み）

**ESCの電源ラインに470μF/25Vの電解コンデンサを追加**
...
**選定ポイント**:
- **容量**: 470μF以上推奨（突入電流の大きさによる）
- **耐圧**: バッテリー電圧の2倍以上（2S LiPo満充電8.4V → 25V以上）
- **種類**: 電解コンデンサ（低ESR品がより効果的）
...
| 2026年1月29日 | **重要**: ESC起動時の突入電流問題と解決策（470μF/25Vコンデンサ追加）を追記。I2C経由のPWM制御について補足 |
| 2026年1月29日 | 現在の構成を明確化: PCA9685 + 付属赤基板ESCで実稼働中。配線図・PWM設定値を追記 |
