...
> **2026-02-08 更新**: スロットル反転バグ（JOYSTICK_THROTTLE_DIR = -1.0）は
> **現在発生していません**。robotcar-engineer より確認済み。
...
1. JOYSTICK_MAX_THROTTLE = 0.5 → 学習データの最大値が0.5
   → モデルは 0.0〜0.5 の範囲でしか学習していない

【結果】
- モデル出力の最大値が約0.5に制限される
- 実際のモデル出力は低い値（0.1〜0.3）に収束

【AI_THROTTLE_MULT の限界】
- 元の値が小さいため、掛け算しても効果が限定的
- 0.15 × 3.0 = 0.45 → ESCデッドバンド付近
- 0.30 × 2.0 = 0.60 → ある程度効果あり
```
...
> **2026-02-08 更新**: スロットル反転バグは発生していないことを確認

学習データの特性（JOYSTICK_MAX_THROTTLE = 0.5）:
- 前進シーン → throttle = 0.3〜0.5 として記録
- 停止シーン → throttle = 0.0 として記録

モデルが学習した内容:
- 「コース画像」→ 0.3〜0.5 のスロットル
- 「壁/停止画像」→ 低いスロットル

**課題**: 最大値が0.5に制限されているため、AI_THROTTLE_MULTで2倍にしても最大1.0程度
...
| 項目 | 状態 |
|------|------|
| 学習データ | JOYSTICK_MAX_THROTTLE = 0.5 で制限 |
| スロットル反転 | **発生していない** ✅ |
| モデル出力 | 低い値（0.1〜0.3） |
| AI_THROTTLE_MULT | 効果限定的（モデル出力が低いため） |
...
- [20260203-1520_モデル推論データ解析レポート.md](20260203-1520_モデル推論データ解析レポート.md) - モデル出力解析
...
| 日付 | 内容 |
|------|------|
| 2026-02-08 17:00 | 初版作成 |
| 2026-02-08 17:15 | スロットル反転バグは発生していないことを確認・更新 |
